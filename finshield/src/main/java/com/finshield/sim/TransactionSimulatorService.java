...
public class TransactionSimulatorService {
    ...
    private static final List<String> IPS = List.of(
            "49.204.12.10", "49.37.88.21", "34.23.11.90", "18.222.10.4",
            "103.21.55.2", "91.198.174.192", "203.0.113.10"
    );

    public void start(UUID accountId, int tps, int fraudRatePercent,
                      boolean rapid, boolean geo, boolean spike, boolean deviceMismatch) {
    ...
    private CreateTransactionRequest buildFraudTxn() {
        ...
        boolean any = state.enableRapidTxn || state.enableGeoAnomaly || state.enableAmountSpike || state.enableDeviceMismatch;
        ...
        for (int attempt = 0; attempt < 6; attempt++) {
            switch (pick) {
                case 0 -> { if (state.enableRapidTxn) return fraudRapidTxn(); }
                case 1 -> { if (state.enableGeoAnomaly) return fraudGeoAnomaly(); }
                case 2 -> { if (state.enableAmountSpike) return fraudAmountSpike(); }
                case 3 -> { if (state.enableDeviceMismatch) return fraudNewDevice(); }
            }
            pick = rnd.nextInt(4);
        }
    ...
    private String randomCountryDifferentFrom(String base) {
        String c;
        do { c = COUNTRIES.get(rnd.nextInt(COUNTRIES.size())); } while (c.equalsIgnoreCase(base));
        return c;
    }
