...
public class RiskQueryService {
    ...
    public RiskDetailsResponse getRisk(UUID transactionId) {
        RiskScore rs = riskScoreRepository.findByTransaction_Id(transactionId)
                .orElseThrow(() -> new NotFoundException("Risk score not found for txn: " + transactionId));

        var signals = fraudSignalRepository.findByTransaction_Id(transactionId).stream()
                .map(s -> new RiskDetailsResponse.SignalItem(
